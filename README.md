...
Slideshow (upstream semantics): F11 or View→Slideshow starts a fullscreen auto-advancing playlist run; Space toggles pause/resume, ↑/↓ step the rate through the 17-preset ladder (250 ms … 60 s), the Slideshow menu picks a preset or opens the Custom dialog (value + ms/s/min unit), manual navigation re-arms the timer, and Esc both leaves fullscreen and pauses the run. With `loop_animations_once` on (the default), the advance holds until the current animation has played through once. While the run is up the status bar's main part reads "Slideshow playing" (outside the Loading/File-not-found/Failed verdicts) and the process holds the display awake (`prevent_sleep`).
