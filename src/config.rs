...
mod tests {
    ...
    fn every_written_key_reads_back() {
        ...
        // 60 int keys + one *_keys line per command (86, with #22's two
        // Everything searches, #37's 22 slideshow rows, #38's 14 animation
        // commands, #39's 9 sort/shuffle/jumpto rows, #41's 5 clipboard
        // commands, #48's command-line options row and #44's 16 panscan
        // rows).
        assert_eq!(c.to_pairs(false).len(), 146, "the upstream save table");
    }
